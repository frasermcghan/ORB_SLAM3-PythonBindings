...
def main(vocab_path, settings_path, sequence_path, association_path):
    ...
    for idx in range(num_images):
        rgb_image = cv2.imread(
            os.path.join(sequence_path, rgb_filenames[idx]), cv2.IMREAD_UNCHANGED
        )
        depth_image = cv2.imread(
            os.path.join(sequence_path, depth_filenames[idx]), cv2.IMREAD_UNCHANGED
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print(
            "Usage: ./orbslam_rgbd_tum path_to_vocabulary path_to_settings path_to_sequence path_to_association"
        )
